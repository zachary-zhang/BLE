...
#define ARCHIVE_PATH_RENAME (@"identifierAndAliasDic.plist") //mBot重命名归档路径
...
-(id)init{
    ...
    if (self) {
        ...
        //add by liuming@15.5.25
        self.lmPeripheralBeanMutArray = [NSMutableArray array];
        ...
        self.identifierAndAliasDic = (NSMutableDictionary *)[NSKeyedUnarchiver unarchiveObjectWithFile:archivePath];
        if (!self.identifierAndAliasDic) {
            self.identifierAndAliasDic = [[NSMutableDictionary alloc] init];
        }
        NSLog(@"self.identifierAndAliasDic = %@",self.identifierAndAliasDic);
    }
    return self;
}
...
-(void)centralManager:(CBCentralManager *)central didDiscoverPeripheral:(CBPeripheral *)peripheral advertisementData:(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI{
    ...
    if ([_activeCentralManager isEqual:central]) {
        ...
        if(![_peripherals containsObject:peripheral]){
            ...
            NSLog(@"添加peripheral _peripherals=%@",_peripherals);
            
            
            LMPeripheralBean *bean = [[LMPeripheralBean alloc] init];
            bean.peripheral = peripheral;
            bean.RSSI = RSSI;
            ...
            [self.lmPeripheralBeanMutArray addObject:bean];
//            NSLog(@"排序前 lmPeripheralBeanMutArray=%@",self.lmPeripheralBeanMutArray);
            
            //按距离：升序排序
            if(self.lmPeripheralBeanMutArray.count>1){
                [self.lmPeripheralBeanMutArray sortUsingComparator:^NSComparisonResult(id obj1, id obj2){
                    return [((LMPeripheralBean *)obj1) distanceByRSSI] > [((LMPeripheralBean *)obj2) distanceByRSSI];
                }];
//                NSLog(@"排序后 lmPeripheralBeanMutArray=%@",self.lmPeripheralBeanMutArray);
            }
        }
        // 更新状态
...
{
    NSLog(@"距离变化了 %@ %@ %@",peripheral.name,peripheral.name,peripheral.RSSI);
}
...
-(void)renameWithIdentifier:(NSString *)identifier andAlias:(NSString *)alias {
    NSString *mAlias = [self.identifierAndAliasDic valueForKey:identifier];
    if (mAlias==nil){
        NSLog(@"首次归档");
    }
    ...
    [self.identifierAndAliasDic setObject:alias forKey:identifier];
    NSLog(@"执行归档 self.identifierAndAliasDic=%@",self.identifierAndAliasDic);
    NSArray *documents = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *docDir = documents[0];
    NSString *archivePath = [docDir stringByAppendingPathComponent:ARCHIVE_PATH_RENAME];
    [NSKeyedArchiver archiveRootObject:self.identifierAndAliasDic toFile:archivePath];
}

-(NSString *)getAliasWithIdentifier:(NSString *)identifier {
   return (NSString *)[self.identifierAndAliasDic objectForKey:identifier];
}
